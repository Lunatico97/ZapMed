...
summarizer = Summarizer(model_card = "DiwasDiwas/t5-small-medicoSummarizer") ;
...
with st.sidebar:
    ...
    if advanced:  
        ...
        def on_change_repitition_penalty():
            st.session_state.length_penalty = repitition_penalty ;
   ...
   # PDF Uploader
    ...
    if uploaded_file is not None:
        ...
        with tab4:
            ...
            with open(f"{base_path}/uploads/summary_{name}.txt" , "w") as f:
                f.write(finalSum) ;
            print('Summary stored at /output directory !!') ;
            f.close() ;
            with st.container(border=True):
                st.subheader("Summarized Text:- ") ;
                st.markdown(finalSum) ;
